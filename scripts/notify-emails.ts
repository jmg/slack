  ...
  for (const user of users) {
    ...
    if (unread > 0) {
      ...
      const ok = await sendEmail({
        to: user.email,
        subject: `You have ${unread} unread message${plural} in Hearth`,
        text: `Hi ${user.name},\n\nYou have ${unread} unread message${plural} waiting.\nOpen ${APP_URL} to catch up.\n\n— Hearth`,
        html: `<p>Hi ${escapeHtml(user.name)},</p><p>You have <strong>${unread}</strong> unread message${plural} waiting.</p><p><a href="${APP_URL}">Open Hearth to catch up →</a></p>`,
      });
      if (ok) emailed += 1;
